...
This rule teaches Aura-Core and Amazon Q to treat resilient networking as a permissioned and auditable workflow. The agent may observe and explain local status, but it must not open channels, transmit packets, sync data, forward messages, or create feedback loops without explicit user interface and operator approval.
...
Until production mode is explicitly enabled, SKYGRID apps and agents should send only diagnostic summaries.
