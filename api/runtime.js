const VERSION = '1.3.9-review-route-coverage';
...
function sendHome(res) {
  return sendHtml(res, 200, 'SKYGRID Runtime', `
    ...
    <p>Aura-Core SKYGRID runtime preflight active. This Vercel deployment serves the API/runtime layer for SKYGRID routes while the public domain can point here or to B12.</p>
    <div class="grid">
...
function getBackendConfig() {
  ...
  return { apiBase, apiKey, emergencyCallId, partnershipCode, configured: Boolean(apiBase && apiKey) };
}
...
function validatePacificHeartPayload(payload) {
  ...
  if (payload.vitals !== undefined && (typeof payload.vitals !== 'object' || Array.isArray(payload.vitals))) errors.push('vitals must be an object when provided.');
  if (payload.alerts !== undefined && !Array.isArray(payload.alerts)) errors.push('alerts must be an array when provided.');
  if (payload.consent !== undefined && typeof payload.consent !== 'object') errors.push('consent must be an object when provided.');
  return errors;
}
...
async function handlePacificHeartIngest(req, res) {
  if (req.method !== 'POST') {
    ...
    return sendJson(res, 405, runtimePayload({ ok: false, service: 'Pacific Heart Emergency Ingest Sandbox', status: 'method_not_allowed', expectedMethod: 'POST' }));
  }
  try {
    const payload = await readJsonBody(req);
    const errors = validatePacificHeartPayload(payload);
    if (errors.length > 0) return sendJson(res, 400, runtimePayload({ ok: false, service: 'Pacific Heart Emergency Ingest Sandbox', status: 'invalid_payload', errors }));
    return sendJson(res, 202, buildPacificHeartHandoff(payload, req));
  } catch (error) {
    return sendJson(res, error.statusCode || 500, runtimePayload({ ok: false, service: 'Pacific Heart Emergency Ingest Sandbox', status: error.statusCode === 413 ? 'body_too_large' : 'request_error', message: error.message || 'Unable to process ingest request.' }));
  }
}
...
async function handleStatus(req, res) {
  ...
  return sendJson(res, 200, runtimePayload({ status: 'healthy', route: '/api/status', backendConfigured: config.configured, backendStatus }));
}
...
async function handleIntake(req, res) {
  ...
  try {
    ...
    return sendJson(res, error.statusCode || 400, runtimePayload({ ok: false, status: error.statusCode === 413 ? 'body_too_large' : 'bad_request', message: error.statusCode === 413 ? 'Request body too large.' : 'Malformed or missing JSON body.' }));
  }
  const config = getBackendConfig();
  if (!config.configured) {
    return sendJson(res, 202, runtimePayload({ status: 'sandbox_accepted', route: '/api/intake', backendConfigured: false, accepted: true, payloadType: typeof payload, guardrails: ['Sandbox acceptance only', 'No backend dispatch performed', 'Configure SKYGRID_API_BASE and SKYGRID_API_KEY for relay mode'] }));
  }
  try {
    ...
    if (response.status === 401 || response.status === 403) return sendJson(res, 401, runtimePayload({ ok: false, status: 'backend_auth_failed' }));
    if (response.status === 400) return sendJson(res, 400, runtimePayload({ ok: false, status: 'backend_validation_failed' }));
    if (!response.ok) return sendJson(res, 502, runtimePayload({ ok: false, status: 'backend_relay_failed' }));
...
export default async function handler(req, res) {
  ...
  if (path === '/api/intake') return handleIntake(req, res);
  if (path === '/api/pacific-heart/ingest') return handlePacificHeartIngest(req, res);
  if (path === '/api/skygrid/helm') return sendJson(res, 200, runtimePayload({ service: 'SKYGRID Helm Status', status: 'operator_assist_ready', command: url.searchParams.get('command') || 'status' }));
  if (path === '/api/skygrid/provenance') return sendJson(res, 200, runtimePayload({ service: 'SKYGRID Provenance Mirror', status: 'pending_or_ready', proofWritten: false }));
  if (path === '/api/skygrid/aws') {
    const config = getBackendConfig();
    return sendJson(res, 200, runtimePayload({ service: 'SKYGRID AWS Mirror', status: config.configured ? 'configured' : 'pending_or_ready', awsConfigured: config.configured, connected: false, roleAssumptionAllowed: false }));
  }
  if (['/dispatch', '/scenarios', '/rates', '/base', '/pay', '/highway', '/interface'].includes(path)) return sendDemoPage(res, path);
  if (path === '/api/pay/quote') return sendJson(res, 200, runtimePayload({ service: 'SKYGRID Pay Quote', status: 'quote_ready', currency: 'USD', amount: 0, note: 'Demo quote endpoint; connect pricing logic before production use.' }));
  if (path === '/api/highway/status') return sendJson(res, 200, runtimePayload({ service: 'SKYGRID Highway', status: 'ready' }));
  if (path === '/api/highway/flasks') return sendJson(res, 200, runtimePayload({ service: 'SKYGRID Highway Flasks', status: 'ready', flasks: [] }));
  if (path === '/api/highway/postman') return sendJson(res, 200, runtimePayload({ service: 'SKYGRID Highway Postman', status: 'ready_for_collection' }));

  return sendJson(res, 404, { ok: false, service: 'Aura-Core SKYGRID Runtime', status: 'not_found', path, sentinel: 'fail_closed', version: VERSION, timestamp: new Date().toISOString() });
}
