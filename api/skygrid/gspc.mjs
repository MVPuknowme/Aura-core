...
export async function evaluateGspcRead({ axis, fetchImpl = fetch, now = () => new Date().toISOString() } = {}) {
  ...
  const response = await fetchImpl(upstream, { method: "GET", headers: { accept: "application/json", "user-agent": "SKYGRID-GSPC-Read/1.0" } });
  const payload = await response.json();
  if (payload?.schema !== EXPECTED_SCHEMA) {
    return blocked(base, 502, "gspc_schema_mismatch", { expected_schema: EXPECTED_SCHEMA });
  }
