...
export async function evaluateGspcRead({ axis, fetchImpl = fetch, now = () => new Date().toISOString(), timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  let response;
  try {
    response = await fetchImpl(upstream, { method: "GET", headers: { accept: "application/json", "user-agent": "SKYGRID-GSPC-Read/1.0" }, signal: controller.signal });
  } catch (error) {
    return blocked(base, error?.name === "AbortError" ? 504 : 502, error?.name === "AbortError" ? "gspc_request_timeout" : "gspc_unreachable");
  } finally {
